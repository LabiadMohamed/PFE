...
const Cart = () => {
  ...
  const handleApplyPromo = () => {
    if (promoCode.trim().toUpperCase() === 'OPTISTYLE20') {
      setDiscount(0.20);
      setPromoMessage({ text: 'Code promo appliqué ! (-20%)', type: 'success' });
  ...
  const goToConfirmation = () => {
    const err = {};
    if (!formData.fullName.trim()) err.fullName = "Smiya darouriya (Nom requis)";
    if (!formData.phone.trim()) err.phone = "Nimirou darouri (Téléphone requis)";
    if (!formData.address.trim()) err.address = "L'adresse complète darouriya";
    
    if (formData.paymentMethod === 'cmi') {
      if (!formData.cardName.trim()) err.cardName = "Requis";
      if (!formData.cardNum.trim()) err.cardNum = "Requis";
      if (!formData.cardDate.trim()) err.cardDate = "Requis";
      if (!formData.cardCvc.trim()) err.cardCvc = "Requis";
    }
  ...
  const renderStepsIndicator = () => (
    <div className="flex items-center justify-center mb-10 w-full max-w-4xl mx-auto px-4">
      {[ { id: 1, label: "Panier" }, { id: 2, label: "Informations" }, { id: 3, label: "Confirmation" } ].map((s, index) => (
  ...
  const renderSummarySidebar = (buttonAction, buttonText, IconLeft, IconRight) => (
    <div className="bg-white p-6 sm:p-8 rounded-2xl w-full lg:w-[400px] h-fit sticky top-28 border border-gray-100 shadow-[0_4px_24px_rgba(41,32,119,0.06)]">
      <h2 className="text-xl font-bold mb-6 text-[#292077] tracking-tight">Résumé de la commande</h2>
      ...
      <div className="space-y-4 mb-6 text-sm text-gray-600">
        ...
        <div className="flex justify-between items-center">
          <span>Livraison</span>
          {isFreeShipping ? (
            <span className="font-semibold text-green-600 uppercase text-xs tracking-wider">Gratuit</span>
          ) : (
      ...
      {step === 1 && (
        <div className="mb-8 p-4 bg-gray-50 rounded-xl border border-gray-100">
          <label className="block text-xs font-semibold text-[#292077] mb-2 uppercase tracking-wide">Code Promo</label>
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="bg-[#292077] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#d4af37] hover:text-white transition-colors"
            >
              Appliquer
            </button>
          </div>
      ...
      <button 
        ...
        className="w-full bg-[#292077] text-white py-4 rounded-xl font-bold text-lg hover:bg-[#d4af37] hover:shadow-lg hover:shadow-[#d4af37]/30 transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#292077]/20 disabled:opacity-50 disabled:cursor-not-allowed group"
      >
    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="flex-1">
        <div className="flex items-end justify-between mb-8">
          <h1 className="text-3xl font-black text-[#292077] tracking-tight">Votre Panier</h1>
        ...
        <div className="space-y-8">
          <section className="bg-white p-6 md:p-8 border border-gray-100 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
            <h2 className="text-lg font-bold text-[#292077] mb-6 flex items-center gap-3">
              ...
              Smiya & Coordonnées (Customer Information)
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Smiya (Nom complet) *</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Nimirou (Téléphone) *</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Ville (Madiina) *</label>
                <select name="city" value={formData.city} onChange={handleInputChange} 
                  ...
                  {["Casablanca", "Rabat", "Marrakech", "Tanger", "Fès", "Agadir", "Oujda", "Autre"].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="sm:col-span-2">
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Adresse complète *</label>
                <textarea name="address" value={formData.address} onChange={handleInputChange} rows="3"
                  className={`w-full p-3.5 bg-gray-50 border ${formErrors.address ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'} rounded-xl focus:outline-none focus:border-[#292077] focus:ring-1 focus:ring-[#292077] focus:bg-white transition-all`} placeholder="Ex: Quartier, Rue, Résidence..."></textarea>
            ...
            <div className="space-y-4">
              <label onClick={() => setFormData(prev => ({ ...prev, paymentMethod: 'cod' }))} className={`block border-2 ${formData.paymentMethod === 'cod' ? 'border-[#292077] bg-[#292077]/5' : 'border-gray-100 hover:border-gray-200 bg-white'} rounded-2xl cursor-pointer transition-all`}>
                ...
                <div className="p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className={`text-base font-bold ${formData.paymentMethod === 'cod' ? 'text-[#292077]' : 'text-gray-900'}`}>Cash on Delivery (Paiement à la livraison)</h3>
                  </div>
                </div>
              </label>
              ...
              <label onClick={() => setFormData(prev => ({ ...prev, paymentMethod: 'cmi' }))} className={`block border-2 ${formData.paymentMethod === 'cmi' ? 'border-[#292077] bg-[#292077]/5' : 'border-gray-100 hover:border-gray-200 bg-white'} rounded-2xl cursor-pointer transition-all`}>
                ...
                <div className="p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1 flex justify-between items-center">
                    <h3 className={`text-base font-bold ${formData.paymentMethod === 'cmi' ? 'text-[#292077]' : 'text-gray-900'}`}>Carte Bancaire (CMI)</h3>
                  </div>
                </div>
                  ...
                  <div className="px-5 sm:px-[72px] pb-6">
                    <div className="bg-white p-5 border border-gray-200 rounded-xl shadow-sm grid grid-cols-2 gap-4">
                      <div className="col-span-2">
                        <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Nom sur la carte</label>
                        <input type="text" name="cardName" value={formData.cardName} onChange={handleInputChange} placeholder="Ex: KARIM EL FASSI" className={`w-full p-3 bg-gray-50 border ${formErrors.cardName ? 'border-red-500' : 'border-gray-200'} focus:border-[#292077] rounded-lg text-sm uppercase`} />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Numéro de carte</label>
                        <input type="text" name="cardNum" value={formData.cardNum} onChange={handleInputChange} placeholder="4000 1234 5678 9010" className={`w-full p-3 bg-gray-50 border ${formErrors.cardNum ? 'border-red-500' : 'border-gray-200'} focus:border-[#292077] rounded-lg text-sm`} />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Date (MM/YY)</label>
              ...
              <label onClick={() => setFormData(prev => ({ ...prev, paymentMethod: 'paypal' }))} className={`block border-2 ${formData.paymentMethod === 'paypal' ? 'border-[#292077] bg-[#292077]/5' : 'border-gray-100 hover:border-gray-200 bg-white'} rounded-2xl cursor-pointer transition-all`}>
                ...
                <div className="p-5 flex items-center gap-4">
                  ...
                  <div className="w-10 h-8 flex items-center justify-center bg-transparent">
    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="flex-1">
        ...
        <h1 className="text-3xl font-black text-[#292077] tracking-tight mb-8 flex items-center gap-3">
          <ClipboardList className="w-8 h-8 text-[#d4af37]" /> Confirm Order (Dernière vérification)
        </h1>

        <div className="space-y-6">
          <div className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Adresse de livraison</h3>
            <p className="text-lg font-bold text-[#292077] mb-1">{formData.fullName}</p>
            <p className="text-gray-600 mb-1">{formData.address}</p>
            <p className="text-gray-600 font-medium">Ville: {formData.city} | Tél: {formData.phone}</p>
          </div>

          <div className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Méthode de Paiement Choisie</h3>
            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl">
              ...
              <div>
                <p className="font-bold text-[#292077] text-lg uppercase">
                  {formData.paymentMethod === 'cod' ? 'Paiement à la livraison' : 
                   formData.paymentMethod === 'cmi' ? 'Carte Bancaire CMI' : 'PayPal'}
                </p>
                {formData.paymentMethod === 'cod' && <p className="text-sm text-gray-500">Préparez le montant exact à la porte.</p>}
                {formData.paymentMethod === 'paypal' && <p className="text-sm text-gray-500">Vous serez redirigé vers PayPal après confirmation.</p>}
              </div>
            </div>
          </div>
  ...
  const renderStep4_Success = () => (
    <div className="mt-12 max-w-2xl mx-auto text-center px-4 animate-in zoom-in-95 duration-700">
      ...
      <h1 className="text-4xl font-black text-[#292077] tracking-tight mb-4">Commande Confirmée ! 🎉</h1>
      <p className="text-lg text-gray-600 mb-8 leading-relaxed">
        Chokran 3la tiqa dyalk f <span className="font-bold text-[#d4af37]">Optistyle</span> ya <span className="font-bold text-gray-900">{formData.fullName}</span> ! <br/>
        Votre commande a bien été validée et enregistrée. 
      </p>
      ...
      <div className="bg-white border-2 border-[#292077]/10 shadow-[0_4px_20px_rgba(41,32,119,0.05)] rounded-3xl p-6 md:p-8 text-left mb-10 max-w-sm mx-auto">
        <h3 className="font-bold text-[#292077] border-b border-gray-100 pb-4 mb-4 text-center text-lg">Résumé de la transaction</h3>
        <div className="space-y-4 text-sm">
          <div className="flex justify-between items-center"><span className="text-gray-500 font-medium">Montant payé:</span><span className="font-black text-lg text-[#d4af37]">{total.toLocaleString('fr-MA')} Dhs</span></div>
          <div className="flex justify-between items-center"><span className="text-gray-500 font-medium">Paiement:</span><span className="font-bold uppercase text-[#292077] bg-gray-100 px-3 py-1 rounded-full">{formData.paymentMethod}</span></div>
          <div className="flex justify-between items-center"><span className="text-gray-500 font-medium">Livré à:</span><span className="font-bold text-gray-900">{formData.city}</span></div>
        </div>
      </div>
